...
fn test_components_init_creates_all_files() {
    ...
    let output = std::process::Command::new("cargo")
        .args(&["run", "--", "components", "init", temp_path.to_str().unwrap()])
        .output()
...
fn test_components_init_skips_existing_files() {
    ...
    fs::write(rust_code_dir.join("index.tsx"), custom_content)
        .expect("Failed to create custom file");
    ...
    let file_content = fs::read_to_string(rust_code_dir.join("index.tsx"))
...
fn test_components_sync_updates_existing_only() {
    ...
    let rust_code_dir = temp_path.join("src/components/RustCode");
    fs::create_dir_all(&rust_code_dir).expect("Failed to create directory");
    fs::write(rust_code_dir.join("index.tsx"), "// Old version")
        .expect("Failed to create file");

    // Run components sync command
    let output = std::process::Command::new("cargo")
        .args(&["run", "--", "components", "sync", temp_path.to_str().unwrap()])
        .output()
        .expect("Failed to run cargo run");
    ...
    let updated_content = fs::read_to_string(rust_code_dir.join("index.tsx"))
        .expect("Failed to read file");
    assert_ne!(
        updated_content, "// Old version",
...
fn test_components_list_without_path() {
    ...
    assert!(stdout.contains("RustCode"), "Should list RustCode component");
    assert!(
...
fn test_components_list_with_path_shows_status() {
    ...
    let rust_code_dir = temp_path.join("src/components/RustCode");
    fs::create_dir_all(&rust_code_dir).expect("Failed to create directory");
    fs::write(rust_code_dir.join("index.tsx"), "// Component")
    ...
    let output = std::process::Command::new("cargo")
        .args(&["run", "--", "components", "list", temp_path.to_str().unwrap()])
        .output()
        .expect("Failed to run cargo run");
...
fn test_components_init_non_docusaurus_project() {
    ...
    let output = std::process::Command::new("cargo")
        .args(&["run", "--", "components", "init", temp_dir.path().to_str().unwrap()])
        .output()
        .expect("Failed to run cargo run");
