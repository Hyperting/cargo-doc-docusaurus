...
enum ComponentsCommand {
    ...
    #[command(long_about = "Install all required React components, CSS files, and theme overrides.\n\
    ...
    #[command(long_about = "Sync (update) only the components that are already installed.\n\
    ...
    #[command(long_about = "Show all available components with their files and types.\n\
    ...
    #[command(long_about = "Display all CSS custom properties (variables) that can be overridden\n\
    ...
    CssVars {
        #[arg(long = "css-only", help = "Output only CSS without explanatory text (for piping to file)")]
        css_only: bool,
    },
}
